...
const PLATFORMS = [
  ...
  { id: 'howto',        label: 'Como Fazer / Setup',      icon: '⚡', file: 'data/howto.js'        },
  ...
  { id: 'microsoft',    label: 'Microsoft Advertising',   icon: '🪟', file: 'data/microsoft.js'    },
  ...
  { id: 'reddit',       label: 'Reddit Ads',              icon: '🟠', file: 'data/reddit.js'       },
  ...
  { id: 'xads',         label: 'X Ads (Twitter)',          icon: '🐦', file: 'data/xads.js'         },
];
...
function updateStats() {
  ...
  // Polling ate todos os modulos carregarem (max 15s)
  let attempts = 0;
  const iv = setInterval(() => {
    attempts++;
    const loaded = PLATFORMS.filter(p => loadedModules[p.id]).length;
    if (loaded === PLATFORMS.length || attempts > 30) {
      clearInterval(iv);
      let total = 0;
      PLATFORMS.forEach(p => {
